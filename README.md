...
- If you run the included `server.py` it exposes an optional `/save-shows` POST endpoint which the UI will call after import to persist `data/shows.json` on disk (useful for local development). For a static-hosted site (GitHub Pages), use the download-and-replace workflow instead.
